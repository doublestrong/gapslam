...
if __name__ == '__main__':
    ...
    for rd_seed in rd_seeds:
        ...
        for case_i, case in enumerate(cases):
            ...
            solver = RBPFSOG(50, 2000, max_drop_mode_weight=0.8)

            run_count = 1
            while os.path.exists(f"{case_dir}/sog{run_count}"):
                run_count += 1
            os.mkdir(f"{case_dir}/sog{run_count}")
            run_dir = f"{case_dir}/sog{run_count}"
            ...
            for i in range(num_batches):
                ...
                if save_res:
                    ...
                    # if len(ng_vars) > 0:
                    #     samples = [key_sample_dict[v] for v in ng_vars]
                    #     samples = np.hstack(samples)
                    #     file = open(f"{step_file_prefix}_ng_ordering", "w+")
                    #     file.write(" ".join([var.name for var in ng_vars]))
                    #     file.close()
                    #     np.savetxt(fname=f"{step_file_prefix}.samples", X=samples)
                    #
                    # v_names = []
                    # for v_i, v in enumerate(solver.vars):
                    #     if v not in key_sample_dict:
                    #         key_sample_dict[v] = np.array(solver.samples[:, solver.var2slice[v]])
