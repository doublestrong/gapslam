...
import yaml
from utils.Visualization import xyzcov2ellipsoidpts, xyzcov2ellipsoid
...
def get_cam_geometries(cam_pose_mats: List[np.ndarray], frustum_vertices, frustum_lines, frustum_colors,
                       traj_color=[0, 1, 0]):
    frustums = []
    cam_points = []
    cam_lines = []
...
if __name__ == "__main__":
    ...
    gray_steps = 50
    old_cam_frustums = None
    gray_step0 = 246

    # remake_steps = [120, 125, 136, 180,230,243]
    ...
    for rd_seed in rd_seeds:
        ...
        # Rx, Ry, Rz,tx, ty, tz
        odom_cov = np.diag(
            np.array([orient_std, orient_std, orient_std, translation_std, translation_std, translation_std]) ** 2)
        ...
        frustum_gray = [[0.5, 0.5, 0.5] for i in range(len(frustum_lines))]
        ...
        for step, f_id in enumerate(kf_ids):
            ...
            if step == 0:
                ...
                if np.linalg.norm(tmp_odom_tf.translation) > delta_trans_tol or abs(
                        np.arccos(np.clip((np.trace(tmp_odom_tf.rotation) - 1) / 2, -1, 1))) > delta_rot_tol:
                    add_lmk = True
            ...
            if add_lmk:
                ...
                if len(obj_seg.masks) > 0:
                    obj_box_masks = np.zeros(
                        (len(obj_seg.masks), obj_seg.masks[0].shape[0], obj_seg.masks[0].shape[1]),
                        dtype=bool)
                    obj_box_masks, obj_yx_mins, obj_yx_maxs, obj_seg_pixels = objmasks2geominfo(obj_seg.masks,
                                                                                                obj_box_masks)
                else:
                ...
                if len(obj_seg.masks) > 0 and ablation_setting["preprocess"]:
                    occluded_ids = objmasks2occludedIDs(obj_seg.masks, obj_box_masks, min_relative_ratio,
                                                        max_occluded_ratio)
                else:
                    occluded_ids = []
                ...
                for obj_i, obj_class in enumerate(obj_seg.obj_cls):
                    ...
                    if obj_score > obj_lo_score and obj_i not in occluded_ids and obj_class in admitted_class:
                        ...
                        if ablation_setting["preprocess"]:
                            relative_x, relative_y = np.clip(abs(x_center - cx) / (img_w / 2), 0, 1), np.clip(
                                abs(y_center - cy) / (img_h / 2), 0, 1)
                        ...
                        if len(same_class_lmks) > 0:
                            mahalanobis_distances = MahalanobisDistances(same_class_lmks, solver, rbt_var, meas_xy,
                                                                         meas_cov, cur_rbt_mat, cur_rbt_mat_inv,
                                                                         cam_intrinsic, cam_model, lmk2jointCov)
                        ...
                        time_s = time.time()
                        if associated_lmk is None and len(diff_class_lmks) > 0 and obj_class in class2lmk_nodes and \
                                ablation_setting["preprocess"]:
                            # reject object detection with wrong semantic labels
                            diff_class_distances = MahalanobisDistances(diff_class_lmks, solver, rbt_var, meas_xy,
                                                                        meas_cov, cur_rbt_mat, cur_rbt_mat_inv,
                                                                        cam_intrinsic, cam_model, lmk2jointCov)
                        ...
                        if associated_lmk is None and len(same_class_ng_lmks) > 0 and ablation_setting["sampleDA"]:
                            ...
                            for tmp_i, lmk_var in enumerate(same_class_ng_lmks):
                                pixels_in_cam = points2pixels(cur_rbt_mat_inv, solver.lmk_samples[lmk_var],
                                                              cam_intrinsic)
                        ...
                        if associated_lmk is None and len(same_class_gs_lmks) > 0 and ablation_setting["sampleDA"]:
                            ...
                            for lmk_var in same_class_gs_lmks:
                                center = solver.gaussian_solver.get_point_estimate(var=to_Key(lmk_var),
                                                                                   vartype=lmk_var.__class__.__name__)
                                ...
                                if sum(pixels_in_box) > 0:
                                    tmp_x_min, tmp_y_min = np.min(
                                        pixels_in_cam[:, np.arange(len(pixels_in_box))[pixels_in_box]],
                                        axis=1).astype(int)
                                    tmp_x_max, tmp_y_max = np.max(
                                        pixels_in_cam[:, np.arange(len(pixels_in_box))[pixels_in_box]],
                                        axis=1).astype(int)
                                    distribution_mask = np.zeros_like(box_mask, dtype=bool)
                                    distribution_mask[tmp_y_min:tmp_y_max + 1, tmp_x_min:tmp_x_max + 1] = True
                                    iou_list.append(1.0 * np.sum(distribution_mask & box_mask) / np.sum(
                                        distribution_mask | box_mask))
                                else:
                                    iou_list.append(0)
            ...
            # if step >= gray_step0 and step <= gray_step0 + gray_steps:  # (step % 10 == 0 and step != 0) or step == len(kf_ids) - 1:
            if True: #step in remake_steps: #True: #step == len(kf_ids) - 1:
                # visualize point clouds
                lmk_pcd = o3d.geometry.PointCloud()
                cur_ng_vars = [v for v in detected_lmks if v not in solver.unimlmk]
                lmk_sample_dict = {}
                if len(cur_ng_vars) > 0:
                    lmk_sample_dict = solver.sample_selected_landmarks(cur_ng_vars, downsample=2000,
                                                                       sample_per_path=100,
                                                                       path_num=100,
                                                                       mixture_comp_num=10)  # solver.lmk_samples
                    viz_colors = [np.tile(lmk2color[v], (len(lmk_sample_dict[v]), 1)) for v in cur_ng_vars]
                    viz_samples = [lmk_sample_dict[v] for v in cur_ng_vars]

                    viz_colors = np.vstack(viz_colors)
                    viz_samples = np.vstack(viz_samples)
                    lmk_pcd.colors = o3d.utility.Vector3dVector(viz_colors)
                    lmk_pcd.points = o3d.utility.Vector3dVector(viz_samples)
                ...
                for v_i, v in enumerate(solver.gs_vars):
                    ...
                    if v in solver.unimlmk:
                        unilmk2ellipsoids[v] = xyzcov2ellipsoid(center=gs_mean[v_i],
                                                                cov=gs_cov[v_i],
                                                                n_std=1.0,
                                                                edgecolor=lmk2color[v],
                                                                resolution=50)

                # file = open(f"{step_file_prefix}_gs_ordering", "w+")
                # file.write(" ".join(v_names))
                # file.close()
                #
                # file = open(f"{step_file_prefix}_gs_mean", "w+")
                # file.write("\n".join([" ".join([str(ele) for ele in arr]) for arr in gs_mean]))
                # file.close()
                #
                # file = open(f"{step_file_prefix}_gs_cov", "w+")
                # file.write("\n".join([" ".join([str(ele) for ele in arr.flatten()]) for arr in gs_cov]))
                # file.close()

                if add_lmk and len(detected_lmks) > 0:
                    # draw detected landmarks in RGB images
                    inv_cur_cam = SE3Pose(cam_pose_mats[-1]).inverse()
                    cur_image = mpimg.imread(obj_seg.img_path)
                    for tmp_lmk in detected_lmks:
                        # tmp_idx = solver.nglmk.index(tmp_lmk)
                        if tmp_lmk in solver.unimlmk:
                            ellipsoid_pts = np.asarray(unilmk2ellipsoids[tmp_lmk].points)
                        else:
                            ellipsoid_pts = lmk_sample_dict[tmp_lmk]
                        ellipse = (inv_cur_cam.mat @ np.hstack([ellipsoid_pts,
                                                                np.ones((len(ellipsoid_pts), 1))]).T)[:3,:]
                        ellipse_in_cam = (cam_intrinsic @ (ellipse / ellipse[-1, :]))[:2, :]
                        valid_idx = (ellipse_in_cam[0, :] >= 0) & (ellipse_in_cam[0, :] < img_w) & (
                                ellipse_in_cam[1, :] >= 0) & (ellipse_in_cam[1, :] < img_h)
                        ellipse_in_cam = ellipse_in_cam[:, valid_idx].astype(int)
                        sample_mask = np.zeros(cur_image.shape[:2], dtype=bool)
                        np.put(sample_mask,
                               np.ravel_multi_index((ellipse_in_cam[1, :], ellipse_in_cam[0, :]), sample_mask.shape),
                               True)
                        # sample_mask[np.ix_(ellipse_in_cam[1, :],ellipse_in_cam[0, :])] = True
                        sample_mask_dilated = ndimage.binary_dilation(sample_mask,
                                                                      [[True, True, True], [True, True, True],
                                                                       [True, True, True]])
                        ellipse_in_cam = np.argwhere(sample_mask_dilated == 1).T
                        valid_idx = (ellipse_in_cam[0, :] >= 0) & (ellipse_in_cam[0, :] < img_h) & (
                                ellipse_in_cam[1, :] >= 0) & (ellipse_in_cam[1, :] < img_w)
                        ellipse_in_cam = ellipse_in_cam[:, valid_idx].astype(int)
                        cur_image[ellipse_in_cam[0, :], ellipse_in_cam[1, :]] = list(lmk2color[tmp_lmk]) + [1]
                    plt.imsave(fname=f"{run_dir}/object_contour{step}.png", arr=cur_image)

                if True:
                    cam_frustum, cam_traj = get_cam_geometries(cam_pose_mats, frustum_vertices=frustum_vertices,
                                                               frustum_lines=frustum_lines,
                                                               frustum_colors=frustum_colors)

                    if step == gray_step0:
                        cam_frustum1, cam_traj1 = get_cam_geometries(cam_pose_mats, frustum_vertices=frustum_vertices,
                                                                   frustum_lines=frustum_lines,
                                                                   frustum_colors=frustum_gray)
                        old_cam_frustums = [cam_frustum1, cam_traj1]


                    # odom_frustum, odom_traj = get_cam_geometries(odom_mats[:step+1], frustum_vertices=frustum_vertices,
                    #                                            frustum_lines=frustum_lines,
                    #                                            frustum_colors=frustum_colors,
                    #                                              traj_color=[0,0,1])
                    # draw_geom = [cam_traj,odom_traj,o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2)] + cam_frustum + ellipsoid_linesets
                    draw_ellipsoid = [tmp_v for tmp_k, tmp_v in unilmk2ellipsoids.items()]
                    draw_geom = [o3d.geometry.TriangleMesh.create_coordinate_frame(
                        size=0.2)] + cam_frustum + draw_ellipsoid
                    if len(lmk_pcd.points) > 0:
                        draw_geom += [lmk_pcd]

                    if old_cam_frustums is not None and step <= gray_step0 + gray_steps:
                        draw_geom += old_cam_frustums[0]

                    vis = o3d.visualization.Visualizer()
                    vis.create_window(visible=False)
                    # vis.create_window(visible=True)
                    for geo in draw_geom:
                        vis.add_geometry(geo)
                    ctr = vis.get_view_control()

                    # for storing camera view points
                    ...
                    vis_params = ctr.convert_to_pinhole_camera_parameters()
                    vis_params.intrinsic.width = vis_json['intrinsic']['width']
                    vis_params.intrinsic.height = vis_json['intrinsic']['height']
                    tmp_arr = np.array(vis_json['extrinsic']).reshape((4, 4)).T
                    vis_params.extrinsic = tmp_arr
                    ctr.convert_from_pinhole_camera_parameters(vis_params, allow_arbitrary=True)
                    vis.poll_events()
                    vis.update_renderer()
                    # time.sleep(0.1)
                    vis.capture_screen_image(filename=f"{run_dir}/step{step}.png", do_render=True)
                    vis.destroy_window()
                    del ctr
                    del vis
